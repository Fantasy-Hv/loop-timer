...
src/
  ...
  overlay.rs    —— 全屏遮罩窗口，每次通知动态创建、确认后销毁
  style.css     —— overlay 窗口 CSS 样式
```

## 配置

首次运行自动生成 `~/.config/loop-timer/config.toml`（可通过 `--config` / `-c` 覆盖）：
...
- `app.run_with_args(&["loop-timer"])` 防止 GTK 解析 `--config` 等自定义参数（参数在 GTK 初始化前由 `std::env::args()` 手动解析）
